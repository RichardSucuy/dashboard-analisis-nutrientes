...
def construir_tabla_html(df):
    ...
    html = '<table class="vertical-headers">\n'
    html += '  <tr>\n'
    for col in columnas:
...
def mostrar_tabla_rangos_editable_suelo():
    nutrientes = list(rangos_suelo_json.keys())
    nombres = [rangos_suelo_json[n]["nombre"] for n in nutrientes]

    st.markdown("### ⚙️ Editar rangos de nutrientes")

    # Crear columnas horizontales para cada nutriente
    ...
    #     col.markdown(
    #         f"<div style='text-align:center; font-weight:bold; writing-mode: vertical-rl; transform: rotate(180deg);'>{nombre}</div>",
    #         unsafe_allow_html=True
    #     )

    # ======= FILA 2: MIN =======
    for col, nutriente in zip(columnas, nutrientes):
        min_default = rangos_suelo_json[nutriente].get("min", 0.0)
    ...
    # ======= FILA 3: MAX =======
    for col, nutriente in zip(columnas, nutrientes):
        max_default = rangos_suelo_json[nutriente].get("max", 0.0)
    ...
    # ======= FILA 4: REFERENCIAL (solo lectura) =======
    for col, min_val, max_val in zip(columnas, min_vals, max_vals):
        ref = round((min_val + max_val) / 2, 2)
        referenciales.append(ref)
        col.markdown(f"<div style='text-align:center; margin-top:6px; font-size: 0.9em; color:#333;'><b>{ref}</b></div>", unsafe_allow_html=True)

    # Guardar en session_state
    st.session_state["valores_referenciales_suelo"] = dict(zip(nutrientes, referenciales))
...
# def mostrar_relacion_bases(valores_promedio: dict):
    ...
    st.markdown("### 🧪 Relación de Bases")

    col_titles = ["Ca/Mg", "Ca/K", "Mg/K", "Ca+Mg/K"]
    col_rangos = st.columns(len(col_titles), gap="small")

    # ===== FILA 1: RANGOS =====
    st.markdown("<div style='text-align:left; font-weight:bold;'>Rangos</div>", unsafe_allow_html=True)
    ...
    # ===== FILA 2: NUTRIENTES INVOLUCRADOS Y RESULTADOS =====
    st.markdown("<div style='text-align:left; font-weight:bold; margin-top:10px;'>Nutrientes involucrados</div>", unsafe_allow_html=True)
...
def mostrar_tabla_saturacion_bases():
    st.markdown("### ⚙️ Saturación de Bases (%)")
